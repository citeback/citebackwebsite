...
Queries state by state, combines, deduplicates, saves to public/alpr-us.json
"""
...
OUT_FILE = os.path.join(os.path.dirname(__file__), '..', 'public', 'alpr-us.json')
...
def fetch_state(state, bbox, retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            req.add_header('User-Agent', 'Deflect ALPR Mapper / OSM Data Collection')
            with urllib.request.urlopen(req, timeout=50) as resp:
                raw = resp.read()
            d = json.loads(raw)
            cameras = [
                {'id': e['id'], 'lat': e['lat'], 'lon': e['lon'], 'tags': e.get('tags', {})}
...
print(f'✓ Saved to public/alpr-us.json ({size_kb} KB)')
print('Done! Reload the map to see all cameras.')
