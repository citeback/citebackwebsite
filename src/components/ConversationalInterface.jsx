...
Direct wallet model: organizers hold their own XMR/ZANO wallets. Citeback never holds funds. Organizers publish a view key so anyone can verify the balance. Early drain = permanent ban. Wallet architecture published before any funds go live. Operated by Scott Hughes. Platform rules on GitHub.
...
const STATIC_RESPONSES = {
  ...
  'Who controls the wallets?':
    'Campaign organizers control their own campaign wallets — that is the direct wallet model. Contributions go straight to the organizer\'s own XMR or ZANO wallet. Citeback never holds keys, never pools funds, and cannot move money. Accountability comes from view key monitoring (anyone can verify the balance in real time), permanent bans for early withdrawal, and identity pre-screening of all campaign organizers. The full architecture is published on GitHub and independently auditable.',
  'Is this legal?':
    'Yes. Funding FOIA litigation, billboard campaigns, and legal defense funds is protected activity under the First Amendment. Monero and Zano are legal to hold and transmit in the United States. No campaign on Citeback may fund illegal activity. Attorney review of regulatory questions is required before any funds are accepted.',
  'When do wallets go live?':
    'Wallets activate once launch prerequisites are met — including written attorney guidance on regulatory questions and deployment of the wallet infrastructure. The current status is visible on the LaunchTracker on the home page. No funds will be accepted until every prerequisite is publicly verified.',
  'What is a Stingray?':
  ...
  'How do I propose a campaign?':
    'Go to the "Run a Campaign" section. Campaign organizers must be verified professionals — attorneys, nonprofits, licensed healthcare professionals, or university/government staff. Verification is automatic against public registries. Your real name is stored privately and never shown publicly. Once verified, you can propose a campaign immediately. The platform operator reviews proposals for legal viability before they go live.',
  'What is ALPR?':
