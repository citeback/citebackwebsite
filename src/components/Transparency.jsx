...
const principles = [
  ...
  { title: 'Receipt Before Release', body: 'Campaign organizers submit vendor receipts and photo proof before any disbursement is triggered. A 48-hour public challenge window follows — the community can contest any release.' },
  { title: 'Citeback Never Holds Funds', body: "Contributions go directly from contributors to the organizer's own wallet — the platform has zero custody and zero spending access. The organizer provides a Monero view key (read-only) so the platform and community can independently verify balances. Early withdrawal before a campaign goal is met results in immediate permanent ban." },
  ...
  { title: 'Zero Platform Fee', body: "Citeback takes nothing from campaigns. No percentage, no deduction, no operations cut. Every dollar contributed goes directly to the campaign organizer's wallet. Operating costs are covered by the founding operator personally. This model was chosen deliberately — a platform that skims campaigns has an extractive relationship with the causes it exists to support." },
  { title: 'Publicly Governed', body: 'Rules are published on GitHub and versioned. Any rule change is announced publicly at least 7 days before it takes effect. The full platform — code, rules, and data — is open source and forkable. Anyone can run their own instance.' },
  { title: 'Transparent Operator', body: "Scott Hughes operates the platform. Responsibilities and limits are published in the platform rules on GitHub. The platform reviews campaign proposals and monitors wallets via view keys — but Citeback never holds wallet keys or campaign funds. Campaign organizers self-custody their own wallets. The direct wallet architecture enforces this by design." },
]
