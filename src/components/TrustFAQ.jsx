...
const sections = [
    ...
    questions: [
      {
        ...
        a: `No human does \u2014 by architectural design, not by promise.\n\nEvery campaign wallet is held by an independent, multi-party wallet system. The operator manages campaigns and reviews proposals. What the operator cannot do is access wallet keys or redirect funds outside the published disbursement rules.\n\nThe platform is being organized as a Wyoming DAO LLC (formation is a Phase 2 launch prerequisite). The full wallet architecture is published openly before any funds go live.`,
      },
      {
        q: 'How does the wallet architecture work?',
        a: `The wallet system is built around a few guarantees:\n\n- No single party holds all keys. Multi-party threshold signatures across minimum 3 independent nodes.\n- Disbursement rules are code, not policy. Published, open-source, auditable before launch.\n- Every disbursement is logged publicly and permanently.\n\nThe full wallet architecture spec is on GitHub. Anyone can audit it before any campaign goes live.`,
      },
      {
        q: 'How do I verify the wallets are legitimate?',
        a: `Every campaign wallet publishes a view key — a read-only key that lets anyone verify the wallet's full transaction history without having spending access. Both Monero (XMR) and Zano (ZANO) wallets support view keys: Monero's view key reveals all incoming transactions to that wallet; Zano's view key provides equivalent read-only transaction visibility.\n\nYou can:\n• Verify the balance matches what the campaign shows\n• Verify every incoming contribution\n• Verify every outgoing disbursement and its destination\n• Cross-reference the disbursement against the public action log\n\nEvery wallet execution will be signed and logged permanently to the public action log on GitHub.`,
      },
      {
        q: 'What happens if a campaign doesn\'t reach its goal?',
        a: `If a campaign deadline arrives with the goal unmet, funds automatically redirect to the highest-priority active campaign in the same category (billboard funds go to another billboard campaign, FOIA funds to another FOIA, etc.).\n\nOperators or community members can request an extension (see Campaign Extensions). Maximum 2 extensions per campaign. After that, redirect is automatic.\n\nAll contributions are final and non-refundable. This is disclosed on every campaign card. Monero's privacy model makes identifying senders technically infeasible without asking donors to partially deanonymize themselves — we won't do that.`,
      },
      {
  ...
  },
  {
    ...
    questions: [
      {
        q: 'Who runs Citeback?',
        a: `Citeback is being organized as a Wyoming DAO LLC (formation is a Phase 2 launch prerequisite). Pre-launch, the founder acts as interim platform operator.\n\nWhat the operator CANNOT do is touch wallet keys — those are held by the independent wallet system, not the operator. The operator has no ability to redirect funds, access campaign wallets, or override disbursement rules. Community governance holds the operator accountable.\n\nPre-launch, the founder acts as the platform operator — reviewing campaign proposals, managing onboarding, and handling OFAC screening — but still cannot access wallet keys (held by the independent wallet system by architectural design). After Phase 2 launch and LLC formation, the founder's account has identical permissions to any other community participant. No veto, no special wallet access, no back door.`,
      },
      {
      ...
      },
      {
        ...
        a: `No — by architectural design.\n\nA human operator (Wyoming DAO LLC, currently in formation) will run the platform — but the operator cannot access funds. After Phase 2 launch:\n• Wallet keys are held in independent, hardware-secured infrastructure — inaccessible to everyone, including the operator and the founder\n• Site changes require community votes that no individual can unilaterally pass\n• Governance rules have 75% supermajority + 14-day time-lock requirements\n• The wallet architecture is publicly verifiable — any tampering is immediately detectable\n\nThe founder bootstrapped the system. The operator keeps it running. The wallet architecture holds the keys. Nobody can redirect the money.`,
      },
      {
  ...
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        a: `The wallet system produces a cryptographic proof of exactly which code is running.\n\nSteps: (1) Pull the current GitHub commit hash. (2) Fetch the wallet attestation from the verification endpoint. (3) Compare the code hashes.\n\nIf they match: the wallet system is running exactly the published code. If they don't: something is wrong and the community should be alerted.\n\nVerification guide and one-click checker will be published at Phase 2 launch.`,
      },
      {
    ...
    questions: [
      {
        ...
        a: `No. Citeback is organized as a Wyoming DAO LLC — a for-profit legal entity. It is not a 501(c)(3) tax-exempt charitable organization.\n\nContributions to Citeback campaigns campaigns are not tax-deductible for US federal income tax purposes. If tax deductibility is important to you, consider the organizations listed in our Frontline Funds section — several are established nonprofits.`,
      },
      {
