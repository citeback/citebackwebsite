...
| 2026-05-18 | `parseCookies()` — `decodeURIComponent` without try/catch: malformed cookie `%ZZ` throws URIError; `verifyAdminSession` + `revokeAdminSession` call `parseCookies` outside try/catch — DoS on any admin route via crafted cookie header | ✅ FIXED (fallback to raw value on URIError, commit a266e62) |
| 2026-05-18 | `reauthSessions` Map missing from GC interval — expired entries (userId→timestamp) never cleaned up; minor memory leak (low severity with 3 users) | ✅ FIXED (added to 10-min GC sweep, commit a266e62) |
...
| 2026-05-11 | ZIP magic bytes validation fails when browser sends empty MIME type (Brave drag-and-drop) | ✅ FIXED (effectiveMime fallback to application/zip when isZip detected via filename + empty MIME, commit 5daff9d) |
