...
**The platform operates under a split operational model.** A Wyoming DAO LLC (the human operator) handles site management, campaign proposal review, operator onboarding, and OFAC pre-screening of operators. A Trusted Execution Environment (TEE) enclave — with no human access to private keys — handles wallet creation, fund custody, automated disbursements per governance rules, and cryptographic attestation. This split model introduces an identifiable legal entity (the Wyoming DAO LLC) into the compliance analysis, which materially simplifies several of the issues described below relative to a fully autonomous AI platform architecture.

**Fee correction (noted for attorney review):** An earlier version of this brief described the platform fee as "1%." This was incorrect. The current graduated fee schedule (GOVERNANCE.md §7.3) is: 5.0% (0–$10k), 4.5% ($10k–$25k), 4.0% ($25k–$50k), 3.0% (above $50k) — all based on 90-day rolling operator volume. The MSB/FinCEN and tax analysis should use these figures.
...
**Operational Responsibilities (Split Model):**  
- *Wyoming DAO LLC (Human Operator):* Site management; campaign proposal review and approval; operator onboarding including OFAC pre-screening; legal compliance; enforcement of campaign taxonomy; community governance oversight; removal of non-compliant content.
- *TEE Enclave (Autonomous):* Wallet creation (XMR and ZANO); private key custody (no human access at any point); automated disbursements per governance rules upon milestone verification; cryptographic attestation of enclave code identity.

This division means that the human operator makes content and operator-eligibility judgments while the TEE makes only financial execution decisions governed by rules the human operator has approved.
...
**Split-Model Impact:**  
The introduction of a Wyoming DAO LLC as the identified human operator changes the MSB analysis in potentially significant ways. A known legal entity is better understood by FinCEN and BSA regulatory frameworks than a fully autonomous, entityless AI platform. The Wyoming DAO LLC can register as an MSB if required, implement a written AML program, and maintain BSA-required recordkeeping for the operator relationships it controls — even if donor-side anonymity remains an architectural constraint. The TEE's automated disbursement function may be analyzable as an agent-of-the-payee or payment processor arrangement flowing through the LLC rather than as a standalone money transmitter. **This does not resolve the question; it changes the framing in ways that may reduce or clarify exposure.**

**Key Question:**  
Does the TEE architecture — in which no human being at any point holds, accesses, or controls wallet private keys, and disbursement is automated via cryptographic proof — eliminate or materially reduce the money transmission nexus under 31 CFR § 1010.100(ff)? Does the platform qualify for the payment processor exemption, the agent of the payee exemption, or any other FinCEN safe harbor? How does the Wyoming DAO LLC's role as identified human operator change the MSB classification analysis, and can an AML program be structured around operator-side verification (which the LLC performs) even where donor anonymity cannot be broken? If the platform is deemed an MSB, what AML program structure is required given that donor anonymity is an architectural constraint that cannot be modified without destroying the platform's core function?

**What We Need:**  
A written legal opinion on MSB classification under current FinCEN guidance and BSA regulations, analyzed against the split model (Wyoming DAO LLC + autonomous TEE). If registration is required, specific guidance on the registration process, minimum AML program elements, and whether an operator-side KYC program (conducted by the LLC) provides adequate BSA compliance where donor-side KYC is architecturally impossible. If an exemption applies, a documented legal basis sufficient to withstand regulatory scrutiny. This opinion should address whether the platform's operators, directors, or technical contributors face personal liability under § 1960 during the pre-launch period. **Recommend obtaining a written MSB opinion directed to the Wyoming DAO LLC by name, which is now feasible given the identified operator.**
...
- TEE compelled-execution analysis (can a court order the TEE to disburse against governance rules?)
